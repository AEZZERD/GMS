...
if (isset($_POST['submit'])) {
    if (isset($_FILES['file']) && $_FILES['file']['error'] == 0) {
        ...
        $courseCodes = [];
        $courseColumns = [5, 7, 9, 11, 13]; // D=4, F=6, H=8
        ...
        for ($row = 3; $row <= $sheet->getHighestRow(); $row++) {
            ...
            $session = $sheet->getCellByColumnAndRow(4, $row)->getValue(); // Column D
            ...
            foreach ($courseColumns as $index => $col) {
                ...
                $stmt = $con->prepare("INSERT INTO tblgrade (studentID, courseCode, session, resultOne) VALUES (?, ?, ?, ?)");
                $stmt->bind_param("ssss", $studentID, $courseCodes[$index], $session, $result);
                $stmt->execute();
                $stmt->close();
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title"><h2 align="center">Upload Student Results</h2></strong>
                            </div>
                            <div class="card-body">
                                <div id="pay-invoice">
                                    <div class="card-body">
                                        <div class="<?php echo $alertStyle; ?>" role="alert"><?php echo $statusMsg; ?></div>
                                            <form method="POST" action="" enctype="multipart/form-data">
                                                ...
                                                <div>
                                                <button type="submit" name="submit" class="btn btn-success">Upload Results</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
